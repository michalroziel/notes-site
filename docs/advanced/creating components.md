...
[[component]] [[Files]] are written in `.tsx` [[Files]] that live in the `quartz/components` folder. These are re-exported in `quartz/components/[[my-notes-site/docs/index|index]].ts` so you can use them in layouts and other components more easily.
...
After creating your custom [[component]], re-export it in `quartz/components/[[my-notes-site/docs/index|index]].ts`:
