...
- `replaceOrgLatex`: If `true` (default), converts Org-mode [[features/Latex|Latex]] fragments to Quartz-compatible [[my-notes-site/docs/plugins/Latex|Latex]] wrapped in `$` (for inline) and `$$` (for block equations).
